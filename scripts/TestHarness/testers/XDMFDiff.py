...
class XDMFDiff(FileTester):
    ...
    def validParams():
        ...
        params.addRequiredParam('xdmfdiff', [], 'A list of XDMF files to compare against the gold.')
        params.addParam('abs_error', None, 'Absolute error threshold.')
        params.addParam('rel_error', None, 'Relative error threshold.')
        params.addParam('abs_zero', None, 'Absolute floor for relative error denominator.')
        params.addParam('abs_tol', None, 'Deprecated alias for abs_error.')
        params.addParam('rel_tol', None, 'Deprecated alias for rel_error.')
        params.addParam('center', 'cell', 'Attribute center(s) to compare: cell, node, or both.')
    ...
    def processResultsCommand(self, moose_dir, options):
        commands = []
        script = os.path.abspath(os.path.join(self.getMooseDir(), '..', 'scripts', 'xdmfdiff.py'))
