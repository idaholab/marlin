...
class TestHarnessTester(MarlinTestHarnessTestCase):
    def testShapeMismatch(self):
        ...
        self.assertRegex(
            e.output,
            r"tester\.shape_mismatch:.*Mismatching shape for dataset 'c\.0' \(gold:\(5, 6\), test:\(9, 9\)\)",
        )
    ...
    def testValueMismatch(self):
        ...
        self.assertRegex(
            e.output,
            r"tester\.value_mismatch:.*Absolute tolerance exceeded in 'c\.0' \(diff:0\.4268.*, abs_tol:1e-15\)",
        )
    ...
    def testDataSetMismatch(self):
        ...
        self.assertRegex(
            e.output,
            r"tester\.dataset_mismatch:.*\['c\.0', 'c\.1', 'c\.2', 'mu\.0', 'mu\.1', 'mu\.2'\]",
        )
