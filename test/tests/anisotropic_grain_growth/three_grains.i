...
c1 = 'inside:=sqrt((x-16)^2+(y-20)^2)-7;(0.5-0.5*tanh(2*inside/${interface_width}))*${left}'
c2 = 'inside:=sqrt((x-24)^2+(y-20)^2)-7;(0.5-0.5*tanh(2*inside/${interface_width}))*(1-${left})'
...
[TensorSolver]
    ...
    nonlinear_reciprocal = 'NL_gr0 NL_gr1 NL_gr2'
    substeps = 1e2
    predictor_order = 1
    corrector_order = 1
    corrector_steps = 1
...
[TensorOutputs]
    [xdmf]
        ...
        buffer = 'gr0 gr1 gr2 sigma kappa_laplacian_gr_i'
