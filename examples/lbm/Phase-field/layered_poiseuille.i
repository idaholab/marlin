...
# mu_l = rho_l * nu_l, mu_g = rho_g * nu_g, h = Ny/2
# uc = 1e-4
# uc = 0.0001
Gx = 0.0000001
...
[TensorComputes/Initialize]
  # Liquid (phi=1) in bottom half (y < Ny/2), gas (phi=0) in top half
  # [phi_init]
  #   type = ParsedCompute
  #   buffer = phi
  #   expression = '0.5 + 0.5 * tanh(2 * (0.5 * Ny - y) / W)'
  #   constant_names = 'Ny W'
  #   constant_expressions = '${Ny} ${W}'
  #   extra_symbols = true
  # []
  [grad_phi_init]
...
[TensorOutputs]
  [xdmf]
    ...
    buffer = 'phi velocity rho pressure forces mu grad_phi laplacian_phi'
    output_mode = 'Cell Cell Cell Cell Cell Cell Cell Cell'
    enable_hdf5 = true
    transpose = false
  []
[]
