...
XDMFTensorOutput::writeLocalData()
{
  ...
  {
    if (!original_buffer->getUnpaddedCPUTensor().defined())
      continue;
    ...
    switch (output_mode)
    {
      ...
      {
        auto extended = extendTensor(original_buffer->getUnpaddedCPUTensor());
        buffer =
      ...
      {
        auto unpadded = original_buffer->getUnpaddedCPUTensor();
...
XDMFTensorOutput::writeSerialXMF()
{
  ...
  for (const auto & [buffer_name, original_buffer] : _out_buffers)
  {
    ...
    const auto sizes = original_buffer->getUnpaddedCPUTensor().sizes();
    if (sizes.size() < _dim)
      mooseError("Tensor has fewer dimensions than specified spatial dimension.");
    ...
    const auto dtype = original_buffer->getUnpaddedCPUTensor().dtype();
...
XDMFTensorOutput::writeParallelXMF()
{
  ...
  for (unsigned int r = 0; r < _n_rank; ++r)
  {
    ...
    for (const auto & [buffer_name, original_buffer] : _out_buffers)
    {
      ...
      const char * dtype_str =
          (original_buffer->getUnpaddedCPUTensor().dtype() == torch::kInt32 ||
           original_buffer->getUnpaddedCPUTensor().dtype() == torch::kInt64)
              ? "Int"
              : "Float";
      const std::string precision =
          (original_buffer->getUnpaddedCPUTensor().dtype() == torch::kFloat64 ||
           original_buffer->getUnpaddedCPUTensor().dtype() == torch::kInt64)
              ? "8"
              : (original_buffer->getUnpaddedCPUTensor().dtype() == torch::kFloat32 ||
                 original_buffer->getUnpaddedCPUTensor().dtype() == torch::kInt32)
                    ? "4"
                    : "1";
