...
LBMPhaseFieldPressure::validParams()
{
  ...
  params.addRequiredParam<TensorInputBufferName>("grad_phi", "Gradient of LBM phase field parameter");
...
LBMPhaseFieldPressure::computeBuffer()
{
  ...
  _u = (f_nonzero_sum + 0.5 * (_rho_l - _rho_g) * u_dot_grad_phi + rho_s_0) * \
       _lb_problem._cs2 / (1.0 - _stencil._weights[0]);
