...
{
  _u_owned.index_put_({0, Slice(), Slice(), Slice()},
                      _u_owned.index({1, Slice(), Slice(), Slice()}));
}

void
LBMMicroscopicZeroGradientBC::rightBoundary()
{
  _u_owned.index_put_({_shape[0] - 1, Slice(), Slice(), Slice()},
                      _u_owned.index({_shape[0] - 2, Slice(), Slice(), Slice()}));
}

void
LBMMicroscopicZeroGradientBC::bottomBoundary()
{
  _u_owned.index_put_({Slice(), 0, Slice(), Slice()},
                      _u_owned.index({Slice(), 1, Slice(), Slice()}));
}

void
LBMMicroscopicZeroGradientBC::topBoundary()
{
  _u_owned.index_put_({Slice(), _shape[1] - 1, Slice(), Slice()},
                      _u_owned.index({Slice(), _shape[1] - 2, Slice(), Slice()}));
}

void
LBMMicroscopicZeroGradientBC::frontBoundary()
{
  _u_owned.index_put_({Slice(), Slice(), 0, Slice()},
                      _u_owned.index({Slice(), Slice(), 1, Slice()}));
}

void
LBMMicroscopicZeroGradientBC::backBoundary()
{
  _u_owned.index_put_({Slice(), Slice(), _shape[2] - 1, Slice()},
                      _u_owned.index({Slice(), Slice(), _shape[2] - 2, Slice()}));
}
